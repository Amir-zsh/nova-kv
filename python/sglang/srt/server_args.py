...
class ServerArgs:
    ...
    def _unified_mixed_kv_active(self) -> bool:
        ...
        if self.speculative_algorithm is not None:
            return False
        # Hybrid SWA models (gpt-oss) now route the unified pool through the
        # full-attention side of SWAKVPool (SWA layers stay bf16), so they
        # take the same page_size = N_Q autoconfig as dense models.
        return True
