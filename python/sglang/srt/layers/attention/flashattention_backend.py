...
class FlashAttentionBackend(AttentionBackend):
    ...
    def forward_extend(
        ...
        if not self.use_mla:
            ...
            #
            # The int2 branch is per-LAYER, not per-backend: on hybrid-SWA
            # models (gpt-oss) only the full-attention layers are quantized;
            # sliding-window layers live in the plain bf16 inner pool and
            # resolve_layer_quant_pool returns None for them. Those layers
            # must take the standard FA3 path below, which handles SWA and
            # sinks natively. Gating on kv_cache_dtype_str alone sent SWA
            # layers into the quant path, where the is_swa assert (correctly)
            # refused to run them.
            _q_pool = None
            _q_layer_id = layer.layer_id
            if self.kv_cache_dtype_str == "int2":
                _q_pool, _q_layer_id = resolve_layer_quant_pool(
                    forward_batch.token_to_kv_pool, layer.layer_id
                )
            if _q_pool is not None:
                ...
                quant_pool, quant_layer_id = _q_pool, _q_layer_id
                ...
                vq_sink_correction = (
                    sinks is not None
                    and getattr(quant_pool, "vq_enabled", False)
                    and not disable_vq_attention_sink_correction()
                )
                if vq_sink_correction:
                    assert layer.logit_cap == 0.0, (
                        "Mean-centered VQ attention is incompatible with "
                        "nonlinear attention-logit capping."
                    )
                    layer_idx = quant_layer_id - quant_pool.start_layer
                    effective_sinks = adjust_vq_attention_sinks(
                        q3,
                        sinks,
                        quant_pool._vq.mean[layer_idx],
                        layer.scaling,
                    )
                ...
                prefix_k, prefix_v = dequantize_prefix_kv(
                    quant_pool, quant_layer_id, prefix_slots, q3.dtype
                )
                ...
                quant_kwargs = kwargs
                if vq_sink_correction:
                    # FA3 accepts only one static sink per head. VQ mean
                    # centering needs a different sink logit for every query,
                    # so request LSE and apply the exact denominator rescale.
                    quant_kwargs = dict(kwargs)
                    quant_kwargs.pop("sinks", None)
                    quant_kwargs["return_softmax_lse"] = True
                ...
                if vq_sink_correction:
                    ...
                    scale = torch.sigmoid(
                        ...
                        - effective_sinks.to(torch.float32).transpose(0, 1)
                    )
                    result = result * scale.transpose(0, 1).unsqueeze(-1).to(
                        result.dtype
                    )
