...
try:
    ...
    # triton_kernels 3.6 deleted the `routing` module (triton-lang/triton 30ede52):
    # the dataclasses moved into matmul_ogs and the entry point moved into callers,
    # rebuilt on top of `topk`. Ported from sglang main's layers/moe/topk.py.
    try:
        from triton_kernels.matmul_ogs import GatherIndx, RoutingData, ScatterIndx

        # sglang main imports this from triton_kernels.tensor, but the vintage
        # bundled by sgl-kernel 0.4.4 re-exports only the RaggedTensorMetadata class.
