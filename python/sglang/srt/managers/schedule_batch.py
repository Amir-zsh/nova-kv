...
class Req(ReqDllmMixin):
    ...
    def __init__(
        ...
        # Per-step HP-recent tail bound; see
        # SGLANG_MIXED_KV_PREFIX_REUSE_ACROSS_CHUNKS.
        self.mixed_kv_hp_tail_cutoff_len: Optional[int] = None
    ...
    def reset_for_retract(self):
        ...
        self.mixed_kv_hp_tail_cutoff_len = None
...
class ScheduleBatch(ScheduleBatchDisaggregationDecodeMixin):
    ...
    def _evict_swa(self, req: Req, pre_len: int):
        ...
        #
        # ``tail_reserve`` is how far short of page_floor(seq_len) the cache's
        # insert boundary actually sits -- 0 for stock SWA, one hp-recent window
        # under mixed-KV, where the tail trim would otherwise leave the frontier
        # ABOVE the insert boundary.
        tail_reserve = self.tree_cache.swa_evict_tail_reserve(pre_len)
        new_swa_evicted_seqlen = max(
            req.swa_evicted_seqlen,
            pre_len - sliding_window_size - self.tree_cache.page_size - tail_reserve,
        )
