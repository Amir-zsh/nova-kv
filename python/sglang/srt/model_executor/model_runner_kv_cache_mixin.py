    ...
    def _init_pools(self: ModelRunner):
        ...
        if self.server_args.attention_backend == "ascend" and not self.mambaish_config:
            ...
            if self.is_hybrid_swa:
                ...
                full_pool_override = None
                if enable_mixed_kv_hybrid:
                    # Hybrid mixed HP+int2: the full-attention layers get the
                    # unified quant+HP pool (dense layer axis 0..N_full-1,
                    # matching layer_map.json-compacted calibration
                    # artifacts); SWA layers stay bf16 below.
                    # Invariant: a windowed layer must only ever read tokens
                    # still resident in the HP-recent ring — otherwise it
                    # would touch a token already flushed to the int2 tier
                    # whose bf16 SWA slot has been freed. That holds iff the
                    # window fits inside the HP-recent window.
                    _swa_win = (
                        self.model_config.sliding_window_size
                        or self.model_config.attention_chunk_size
                    )
                    _hp_recent = envs.SGLANG_MIXED_KV_RECENT_TOKENS.get()
                    assert _swa_win is not None and _swa_win <= _hp_recent, (
                        f"Hybrid mixed KV requires sliding_window_size "
                        f"({_swa_win}) <= SGLANG_MIXED_KV_RECENT_TOKENS "
                        f"({_hp_recent}); otherwise windowed layers read "
                        f"flushed tokens with freed SWA slots."
                    )
                    hp_dtype = resolve_hp_dtype(envs.SGLANG_MIXED_KV_HP_DTYPE.get())
                    hyb_scale_dtype = resolve_scale_dtype(
                        envs.SGLANG_MIXED_KV_SCALE_DTYPE.get()
                    )
                    n_h, n_q = compute_page_geometry(hp_dtype)
                    assert self.page_size == n_q, (
                        f"Hybrid unified mixed KV requires page_size={n_q} "
                        f"(= N_Q), got page_size={self.page_size}"
                    )
                    num_quant_pages = (
                        self.full_max_total_num_tokens + n_q - 1
                    ) // n_q + 1
                    ...
                    logger.info(
                        "Enable hybrid unified mixed KV (int2, full-attention "
                        "layers only): n_full_layers=%s prefix=%s recent=%s "
                        "num_quant_pages=%s N_Q=%s hp_dtype=%s "
                        "full_max_total_num_tokens=%s hp_prefix_pool_tokens=%s",
                        n_full_layers,
                        p_tokens,
                    ...
                    full_pool_override = UnifiedInt2HPKVPool(
                        ...
                        layer_num=n_full_layers,
                        device=self.device,
                        enable_memory_saver=self.server_args.enable_memory_saver,
                        max_req_slots=self.req_to_token_pool.size,
                        start_layer=0,
                        end_layer=n_full_layers,
                        model_dtype=self.dtype,
                        kv_cache_quant_group_size=(
                            self.server_args.kv_cache_quant_group_size
                        ),
                        scale_dtype=hyb_scale_dtype,
                    )

                swa_pool_dtype = self.kv_cache_dtype
                full_pool_dtype = None
                if self.kv_cache_dtype == "int2":
                    # Hybrid-SWA int2: quantize only the full-attention
                    # layers (dense-reindexed 0..N_full-1, matching the
                    # dense layer axis of the calibration artifacts /
                    # layer_map.json); SWA layers hold at most window_size
                    # tokens and stay in the model dtype.
                    swa_pool_dtype = self.dtype
                    full_pool_dtype = "int2"
                    kwargs["model_dtype"] = self.dtype
                    kwargs["kv_cache_quant_group_size"] = (
                        self.server_args.kv_cache_quant_group_size
                    )
                    kwargs["scale_dtype"] = resolve_scale_dtype(
                        envs.SGLANG_MIXED_KV_SCALE_DTYPE.get()
                    )
                    logger.info(
                        "Hybrid SWA int2 KV cache: full-attention layers "
                        "int2 (scale_dtype=%s), SWA layers %s",
                        envs.SGLANG_MIXED_KV_SCALE_DTYPE.get(),
                        self.dtype,
                    )
                self.token_to_kv_pool = SWAKVPool(
                    ...
                    size_swa=self.swa_max_total_num_tokens,
                    page_size=self.page_size,
                    dtype=swa_pool_dtype,
                    full_dtype=full_pool_dtype,
                    full_kv_pool_override=full_pool_override,
                    head_num=self.model_config.get_num_kv_heads(
                        get_attention_tp_size()
                    ),
                    head_dim=self.model_config.head_dim,
