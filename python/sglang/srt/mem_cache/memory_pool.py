...
class MHATokenToKVPool(KVCache):
    ...
    def set_kv_buffer(
        ...
        # Simulated quantization (default off). Applied to post-RoPE K/V on the BF16 path
        # before the store, so prefill AND decode writes both go through it -- this method
        # is the single write funnel and carries `is_decode`. The dequantized values are
        # stored as BF16, which is exactly what a real pool of that bit-width would feed the
        # attention kernels; only the memory saving is absent. Lets us measure widths this
        # stack has no pool for (no int4 path) on generation-dominated reasoning tasks.
        # Unset SGLANG_SIMQUANT_PATH -> this block is skipped entirely.
        _simquant = envs.SGLANG_SIMQUANT_PATH.get()
        if _simquant and not (
            getattr(self, "_is_swa_inner_pool", False)
            and envs.SGLANG_SIMQUANT_SKIP_SWA.get()
        ):
            from sglang.srt.mem_cache.sim_quant import apply_sim_quant

            cache_k, cache_v = apply_sim_quant(
                _simquant, layer_id - self.start_layer, cache_k, cache_v
            )
        ...
        if self.dtype == "int2":
            ...
            if lloyd_max and envs.SGLANG_INT2_NO_HADAMARD.get():
                # TurboQuant-INT2 is defined as rotation + Lloyd-Max; refuse
                # the rotation-free combination instead of silently serving
                # an undefined baseline. (Pre-rotated prefill writes are fine
                # — the plain writer has the same LM branch.)
                raise RuntimeError(
                    "SGLANG_LLOYD_MAX=1 requires Hadamard rotation "
                    "(unset SGLANG_INT2_NO_HADAMARD)"
                )
            if already_hadamard_transformed or envs.SGLANG_INT2_NO_HADAMARD.get():
                # No-Hadamard (Naive-INT2 baseline) or already-rotated: store K/V as-is with
                # plain int2 (no in-pool rotation). For Naive, apply_segmented_hadamard_transform
                # is also a no-op everywhere, so K/V arrive raw and attention stays in raw space.
