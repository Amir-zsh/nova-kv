...
class UnifiedInt2HPKVPool(KVCache):
        ...
        with self.memory_saver_adapter.region(GPU_MEMORY_TYPE_KV_CACHE):
            with (
                ...
                if self.vq_enabled:
                    ...
                    # (G=4, 2 b/coord) they fit in uint8 -> 2.0 b/coord of index
                    # memory. int16 doubled the arena for no information gain.
                ...
                if self.vq_enabled:
                    # Slot 0 holds the per-token ptn RMS scale; slot 1 stays 0
                    # (same [*, H, 2] single-scale layout as int2, so decode
                    # strides are unchanged). +1 trash row, matching k_buffer.
    ...
    def _vq_write_k(
        ...
        self._vq_k_idx_big[layer_idx].index_copy_(0, loc_safe, idxs)
        sz = torch.zeros(
            (r_k.shape[0], self.head_num, 2),
            dtype=self.scale_dtype,
            device=r_k.device,
        )
        sz[..., 0] = scale.to(self.scale_dtype)
        self._vq_k_sz_big[layer_idx].index_copy_(0, loc_safe, sz)
    ...
    def _vq_write_v(
        ...
        fixed quarter layout lines up (see ``_vq_v_perm``)."""
        if r_v.shape[0] == 0:
            return
        trash = self.quant_size
        loc64 = loc.to(torch.int64)
        ...
        self._vq_v_idx_big[layer_idx].index_copy_(0, loc_safe, idxs)
        sz = torch.zeros(
            (r_v.shape[0], self.head_num, 2),
            dtype=self.scale_dtype,
            device=r_v.device,
        )
        sz[..., 0] = scale.to(self.scale_dtype)
        self._vq_v_sz_big[layer_idx].index_copy_(0, loc_safe, sz)
    ...
    def vq_flush_k(self, plan) -> None:
        ...
        if envs.SGLANG_VQ_OPT_FLUSH.get():
            ...
            self._vq_k_idx_big.index_copy_(1, dst, idxs)
            sz = torch.zeros(
                (L, n, H, 2), dtype=self.scale_dtype, device=r.device
            )
            sz[..., 0] = scales.to(self.scale_dtype)
            self._vq_k_sz_big.index_copy_(1, dst, sz)
            return

        idxs = torch.empty(
            (L, n, H, vq.num_groups), dtype=torch.uint8, device=r.device
        )
        ...
        for t0 in range(0, n, 8):
            ...
            idxs[:, t0:t1] = sco.argmax(-1).to(torch.uint8)
        self._vq_k_idx_big.index_copy_(1, dst, idxs)
        sz = torch.zeros(
            (L, n, H, 2), dtype=self.scale_dtype, device=r.device
        )
    ...
    def vq_flush_v(self, plan) -> None:
        ...
        for t0 in range(0, n, 8):
            ...
            sco -= vq.cb_sq.unsqueeze(1)
            idxs[:, t0:t1] = sco.argmax(-1).to(torch.uint8)
        self._vq_v_idx_big.index_copy_(1, dst, idxs)
        sz = torch.zeros((L, n, H, 2), dtype=self.scale_dtype, device=r.device)
        sz[..., 0] = scales.to(self.scale_dtype)
        self._vq_v_sz_big.index_copy_(1, dst, sz)
