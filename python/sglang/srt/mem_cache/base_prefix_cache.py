...
class BasePrefixCache(ABC, PrefixCacheTrait):
    ...
    def swa_evict_tail_reserve(self, committed_len: int) -> int:
        """Tokens at the tail that ``_evict_swa`` must not release SWA for.

        ``_evict_swa`` keeps its frontier below the tree's insert boundary so
        every inserted node has non-tombstone SWA behind it. Caches whose
        insert boundary sits short of ``page_floor(seq_len)`` report the gap
        here; see the override in ``SWARadixCache``.
        """
        return 0
