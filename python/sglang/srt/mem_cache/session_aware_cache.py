...
class SessionAwareCache(BasePrefixCache):
    ...
    # Explicit, not via __getattr__: BasePrefixCache defines a 0 default, so
    # normal lookup would succeed here and silently disable the inner cache's
    # reserve rather than forwarding to it.
    def swa_evict_tail_reserve(self, committed_len: int) -> int:
        return self.inner.swa_evict_tail_reserve(committed_len)
