...
# ---------------------------------------------------------------------------
# Mixed-KV helpers, shared by RadixCache, SWARadixCache and ChunkCache.
#
# These live at module scope rather than on a base class because they are tree
# primitives, not part of the BasePrefixCache contract -- the same reason
# ``page_align_keys`` above is a free function. They were duplicated once
# already (ChunkCache), and SWARadixCache silently never got them at all, which
# is the bug this exists to prevent recurring: HP-recent slot ids are per-request
# and aliased across requests, so a cache that inserts them hands one request
# another's live KV.
#
# NOTE: these unify the *mechanism*. The *policy* -- which helper is called at
# which point in cache_unfinished_req -- necessarily differs between RadixCache
# and SWARadixCache, because SWA's _insert_helper frees overlapped indices
# internally and RadixCache's does not. Keep the two sequences cross-referenced.
# ---------------------------------------------------------------------------


def mixed_kv_detect(token_to_kv_pool_allocator) -> Tuple[bool, int, int]:
    """Return (enabled, hp_prefix_tokens, match_cap_overhead) for a KV allocator."""
    if token_to_kv_pool_allocator is None:
        return False, 0, 0
    kvc_getter = getattr(token_to_kv_pool_allocator, "get_kvcache", None)
    ...
    if mixed_kv_enabled_fn is None or not mixed_kv_enabled_fn():
        return False, 0, 0
    hp_prefix_tokens = int(getattr(kvc, "hp_prefix_tokens", 0))
    hp_recent = int(getattr(kvc, "hp_recent_tokens", 0))
    flush_overflow = max(0, int(getattr(kvc, "flush_interval", 1)) - 1)
    return True, hp_prefix_tokens, hp_recent + flush_overflow


def mixed_kv_match_cap(
    key_len: int, hp_prefix_tokens: int, cap_overhead: int, page_size: int
) -> int:
    """Longest prefix that is safe to share, i.e. excludes the HP-recent band."""
    cap = min(key_len, max(hp_prefix_tokens, key_len - cap_overhead))
    if page_size > 1:
        cap = cap // page_size * page_size
    return cap


def mixed_kv_tail_to_drop(
    token_to_kv_pool_allocator, page_size: int, committed_len: int
) -> int:
    ...
    if page_size > 1:
        trim = math.ceil(trim / page_size) * page_size
    # Clip back if ceil pushed past the available range.
    trim = min(trim, committed_len - hp_prefix)
    return trim


def mixed_kv_with_quant_slack(req: Req, indices: torch.Tensor) -> torch.Tensor:
    ...
    req.mixed_kv_hp_tail_cutoff_len = None
    return torch.cat([indices.to(torch.int64), slack.to(indices.device)])


def mixed_kv_slack_insert_limit(req: Req, key_len: int) -> int:
    """Keep radix ownership below any request-owned partial quant page."""
    bounds = [
        b
        for b in (
            getattr(req, "mixed_kv_quant_slack_cutoff_len", None),  # partial page
            getattr(req, "mixed_kv_hp_tail_cutoff_len", None),  # HP-recent tail
        )
        if b is not None
    ]
    if not bounds:
        return key_len
    return max(0, min(key_len, min(int(b) for b in bounds)))
...
class RadixCache(BasePrefixCache):
    ...
    def _with_mixed_quant_slack(self, req: Req, indices: torch.Tensor) -> torch.Tensor:
        return mixed_kv_with_quant_slack(req, indices)

    def _mixed_kv_slack_insert_limit(self, req: Req, key_len: int) -> int:
        return mixed_kv_slack_insert_limit(req, key_len)
