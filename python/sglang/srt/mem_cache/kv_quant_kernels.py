...
def _quantized_set_kv_int2_kernel(
    ...
    if LLOYD_MAX:
        # Lloyd-Max INT2 with the MSE-optimal uniform-level encoding — must
        # stay bit-identical to the LM branch of the fused Hadamard writer
        # (fused_hadamard_int2_kv.py): prefill writes pre-rotated K/V through
        # THIS kernel while decode writes through the fused one, and both
        # feed the same readers. See turbo_lm_check.py for the constants.
