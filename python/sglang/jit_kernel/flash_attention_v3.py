...
def flash_attn_with_kvcache(
    ...
    # Keyword, not positional: the installed sgl-kernel wheel may carry
    # parameters the vendored source predates (e.g. `only_qv`, added before
    # `sm_margin`). Positional forwarding silently shifts every later argument
    # by one, which turned `return_softmax_lse` into `sm_margin` and dropped
    # `sinks` -- a silent wrong-answer, not an error.
...
def flash_attn_varlen_func(
    ...
    # Keyword, not positional -- see the note in flash_attn_with_kvcache above.
