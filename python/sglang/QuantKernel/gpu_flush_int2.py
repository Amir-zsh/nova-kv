...
def _fused_flush_quant_kernel(
    ...
    # K side. Skipped entirely under the vq2 K quant tier: the caller runs a
    # torch-side VQ encode over the same plan tensors instead (the int2 pack
    # would write uint8 crumbs into what is then an int16 index arena).
    if not K_VQ:
    ...
    # V side. Skipped under the vq2 V quant tier (VQ-V ablation), same as K:
    # the caller runs a torch-side VQ encode over the plan tensors instead.
    if not V_VQ:
...
def gpu_flush_int2_apply(
    ...
    *,
    ...
    ``k_vq=True`` skips the in-kernel K int2 pack (vq2 K tier); the caller is
    then responsible for VQ-encoding the flushed K rows from the same plan.
