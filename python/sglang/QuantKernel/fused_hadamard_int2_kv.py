...
def _make_fused_kernel_int2(head_dim: int, hadamard_order: int):
    ...
    def _fused_hadamard_int2_set_kv_kernel(
        ...
        if LLOYD_MAX:
            # TurboQuant baseline: Lloyd-Max INT2 on per-(token, head)
            # standardized coords. Indices come from the LM-optimal
            # thresholds {-T, 0, T}*std around the row mean; the stored
            # (scale, zero) encode the MSE-OPTIMAL uniform-level fit to the
            # LM centroids so every reader keeps the plain
            # `(q - zero) * scale` contract (no exact-centroid dequant
            # anywhere — that path destabilized production decode, see
            # oscar_rotation_clip_int2_kv.py):
            #   levels = (q - 1.5) * DELTA * std + mean,
            #   DELTA* = E[x*l(x)]/E[l(x)^2] = 0.98774 for x~N(0,1)
            # which sits within +1.2% of exact-LM distortion (analytic and
            # Monte-Carlo in pipelines/oscar_e2e/turbo_lm_check.py) vs +26%
            # for the legacy dynamic-range-matched RATIO=1.16 encoding.
            LM_T: tl.constexpr = 0.9810652732849121
            LM_DELTA: tl.constexpr = 0.9877367723552428
            n_sum = (
                tl.sum(tl.where(dim_mask, vals0, 0.0), axis=0)
                + tl.sum(tl.where(dim_mask, vals1, 0.0), axis=0)
                + tl.sum(tl.where(dim_mask, vals2, 0.0), axis=0)
                + tl.sum(tl.where(dim_mask, vals3, 0.0), axis=0)
            )
            row_mean = n_sum / head_dim_
            d0 = tl.where(dim_mask, vals0 - row_mean, 0.0)
            d1 = tl.where(dim_mask, vals1 - row_mean, 0.0)
            d2 = tl.where(dim_mask, vals2 - row_mean, 0.0)
            d3 = tl.where(dim_mask, vals3 - row_mean, 0.0)
            row_var = (
                tl.sum(d0 * d0, axis=0) + tl.sum(d1 * d1, axis=0)
                + tl.sum(d2 * d2, axis=0) + tl.sum(d3 * d3, axis=0)
            ) / head_dim_
            row_std = tl.sqrt(row_var + 1e-8)
            thr = LM_T * row_std
            q0 = ((d0 >= -thr).to(tl.uint8) + (d0 >= 0.0).to(tl.uint8)
                  + (d0 >= thr).to(tl.uint8))
            q1 = ((d1 >= -thr).to(tl.uint8) + (d1 >= 0.0).to(tl.uint8)
                  + (d1 >= thr).to(tl.uint8))
            q2 = ((d2 >= -thr).to(tl.uint8) + (d2 >= 0.0).to(tl.uint8)
                  + (d2 >= thr).to(tl.uint8))
            q3 = ((d3 >= -thr).to(tl.uint8) + (d3 >= 0.0).to(tl.uint8)
                  + (d3 >= thr).to(tl.uint8))
            scale = LM_DELTA * row_std
            zero = 1.5 - row_mean / scale
        else:
...
def quantized_set_kv_int2_hadamard_fused_triton(
    ...
    for (data, buf, sz_buf) in [
        ...
        if lloyd_max and num_groups != 1:
            raise ValueError(
                "SGLANG_LLOYD_MAX int2 needs per-(token, head) scales "
                f"(quant group == head_dim); got {num_groups} scale groups"
            )
        if num_groups == 1:
    ...
    for (data, buf, sz_buf) in [
        ...
        if num_groups == 1:
            ...
            _quantized_set_kv_int2_kernel[grid](
                ...
                # The kernel gained LLOYD_MAX (no default) when the TurboQuant
                # write path landed, but only the clipped writer above was
                # updated. This pretransformed writer takes no lloyd_max
                # argument, so the plain uniform encoding is the correct value.
                LLOYD_MAX=False,
                num_warps=1,
                num_stages=1,
            )
