...
def _pretransformed_int2_set_kv_clip_single_kernel(
    ...
    if LLOYD_MAX:
        ...
        # MSE-optimal uniform-level fit to the LM centroids:
        #   levels = (q - 1.5) * DELTA * std + mean,
        #   DELTA* = E[x*l(x)]/E[l(x)^2] = 0.98774 for x~N(0,1),
        # within +1.2% of exact-LM distortion (derivation + Monte-Carlo in
        # pipelines/oscar_e2e/turbo_lm_check.py). Replaces the legacy
        # LM_RATIO=1.16 dynamic-range matching, which inflated the levels
        # ~18% past optimal and cost +26% distortion vs exact LM.
        LM_DELTA: tl.constexpr = 0.9877367723552428
        uniform_scale = LM_DELTA * row_std
        uniform_zero = 1.5 - row_mean / uniform_scale
    else:
